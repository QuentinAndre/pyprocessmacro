...
- Multicategorical X and moderators, PROCESS's `mcx` and `mcw` options (`mcz`, `mcv`, `mcq` for the other moderator symbols): indicator, sequential, Helmert and effect coding; relative direct, conditional direct, indirect and conditional indirect effects per code; omnibus tests of the direct effect and of the equality of conditional means; the index of moderated mediation per code of X or of the moderator; an `X` column in the tables and an `x_code` column in `tidy()`. Groups may be numbers, strings or a pandas Categorical (also #24). Checked against PROCESS for R 5.0 on 23 cases (`tests/Results/v5/mc`, generated by `tests/fixtures/regenerate.py --suite mc`). Not yet with a multicategorical variable: the two-moderator indices, the floodlight analysis, the plots and `contrast` (#17).
