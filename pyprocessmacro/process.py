...
class Process(object):
    ...
    def _gen_spotlight_values(self, modval_symb):
        ...
        for mod in self._moderators["all"]:
            ...
            elif rule == "percentiles":
                spot_values[mod] = np.percentile(val, [16, 50, 84], method="weibull")
            else:
                m = val.mean()
                std = val.std(ddof=1)
    ...
    def _repr_html_(self):
        ...
        html = [f"<h3>PROCESS Model {self.model_num}</h3>", f"<p>{self._conventions_text()}</p>"]
        if self.options["detail"]:
